...
import { Buttons } from "@shared"; // Твои кнопки из shared
import { ChatButton, ChatsDTO } from "@/entities/chats";
import { UserBadge, UserDTO } from "@/entities/user";
...
export const Sidebar = ({
    ...
    return (
        <aside className={clsx(
            "flex h-full w-[260px] shrink-0 flex-col bg-[#F9F9F9] p-3 border-r border-gray-200",
            className
        )}>
            ...
            <Buttons.IconButtonBoth
                onClick={onNewChat}
                className="bg-[#F5F5F5] rounded-md mb-5 cursor-pointer"
            ...
            <div className="flex-1 overflow-y-auto overflow-x-hidden -mx-2 px-2 space-y-1 custom-scrollbar">
                ...
                {isLoading ? (
                    Array.from({ length: 5 }).map((_, i) => (
                        <div key={i} className="h-10 w-full bg-gray-200/50 rounded-md animate-pulse mb-2" />
                    ))
                ) : (
                    chats.map((chat) => (
                        <ChatButton
